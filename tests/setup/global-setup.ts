 ...
 * process.env) never resolve to the production ~/.walnut/ directory.
 *
...
export function setup(): void {
  const prodHome = path.join(os.homedir(), '.walnut'); // safe: production-path — comparison only
  const current = process.env.WALNUT_HOME;
  ...
  const testHome = path.join(os.tmpdir(), 'walnut-test-global');
