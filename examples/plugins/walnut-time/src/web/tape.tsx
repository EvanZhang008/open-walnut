...
import { TAPE_PX_PER_MIN, layoutTape } from './time-views'
...
export function TimeTape({ slices, rows, axis, minuteOf, nowMin, labelFor }: {
  ...
  return (
    <div className="wt-tp" data-testid="time-app-tape">
      <div className="wt-tp-gutter" style={{ height }}>
        {axis.hours.map((h) => (
          <span
            key={h}
            className="wt-tp-hour"
      ...
      <div className="wt-tp-rank" data-testid="time-app-tape-rank">
        <h4>Where it went</h4>
        {groups.main.map((row) => (
          <RankRow key={row.taskId} row={row} hoverTaskId={hoverTaskId} onHover={setHoverTaskId} />
        ))}
        {groups.hidden.length > 0 && !expanded && (
          <button className="wt-tp-more" data-testid="time-app-tape-more" onClick={() => setExpanded(true)}>
        ...
        {expanded && (
          <button className="wt-tp-more" onClick={() => setExpanded(false)}>Show fewer</button>
        )}
        {groups.quick.length > 0 && (
          <div className="wt-tp-rrow is-quick" data-testid="time-app-tape-quick">
            <i className="wt-tp-dot wt-tp-dot-quick" />
            <span className="wt-tp-rname">Quick touches · {groups.quick.length} tasks</span>
            <span className="wt-tp-rtime">{formatDuration(groups.quickMs)}</span>
          </div>
        )}
      </div>
    </div>
...
function RankRow({ row, hoverTaskId, onHover }: {
  ...
  return (
    <div
      className={`wt-tp-rrow${hoverTaskId === row.taskId ? ' is-lit' : ''}`}
      data-testid="time-app-tape-rrow"
      data-time-task-id={row.taskId}
      onMouseEnter={() => onHover(row.taskId)}
      onMouseLeave={() => onHover(null)}
    >
      <i className="wt-tp-dot" style={{ background: taskColor(row.taskId) }} />
      {/* Full title in the tooltip: one line is the layout, not the content. */}
      <span className="wt-tp-rname" title={row.title}>{row.title}</span>
      <span className="wt-tp-rtime">{formatDuration(row.ms)}</span>
    </div>
  )
}
...
export function Ribbon({ slices, axis, minuteOf, nowMin, labelFor, hoverTaskId, onHover, pxPerMin, testId }: {
  ...
  return (
    <div className="wt-tp-ribbon" data-testid={testId ?? 'time-app-tape-ribbon'} style={{ height }}>
      ...
      {axis.hours.slice(1).map((h) => (
        <i key={h} className="wt-tp-rule" style={{ top: (h * 60 - axis.startMin) * scale }} />
      ))}
      ...
      {segments.map((seg, i) => {
        ...
        return (
          <div
            ...
            onMouseLeave={() => onHover(null)}
          >
            {seg.label > 0 && <span className="wt-tp-seg-title">{title}</span>}
      ...
      {nowMin !== null && showNow && (
        <div
          className="wt-tp-now"
          data-testid="time-app-tape-now"
          style={{ top: (nowMin - axis.startMin) * scale }}
        >
          <i />
