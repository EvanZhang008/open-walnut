 ...
 * relative to what it saw (`base`), laid over `fresh`. Scalars, arrays, and
 * `null` (delete the section) pass through as written.
 */
export function rebaseOnto(partial: Partial<Config>, base: Config, fresh: Config): Partial<Config> {
  ...
  for (const [key, next] of Object.entries(partial as Obj)) {
    const was = b[key];
    const now = f[key];
    if (!isPlainObject(next) || !isPlainObject(was) || !isPlainObject(now)) {
      out[key] = next;
      continue;
    }
    ...
    for (const sub of new Set([...Object.keys(was), ...Object.keys(next)])) {
      ...
      if (!same(nextVal, was[sub])) merged[sub] = nextVal;
    }
    out[key] = merged;
  }
