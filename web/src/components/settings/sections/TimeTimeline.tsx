...
import { fetchTimeBlocks, type DayBlocks, type TimeBlock, type TimeKind } from '@/api/time';
...
import { TimeChapters } from './TimeChapters';
import { TimeLanes } from './TimeLanes';
import '@/styles/time-timeline.css';
import '@/styles/time-views.css';
...
/** Remembers the sub-view and the agents toggle. `open-walnut-` ⇒ ui-prefs syncs it. */
const LS_VIEW_KEY = 'open-walnut-time-timeline-view';
const LS_AGENTS_KEY = 'open-walnut-time-timeline-agents';
/** Tallest the vertical views get before they scroll instead of compressing hours. */
const MAX_SCROLL_PX = 620;
...
export function TimeTimeline({ dates, today, titleFor }: {
  ...
  useEffect(() => {
    let live = true;
    Promise.all([
      fetchTimeBlocks(date, { raw: true, kinds: HUMAN_KINDS }),
  ...
  return (
    ...
    >
      <div className="tt-nav">
        ...
        <div className="tt-switch" role="tablist" aria-label="Timeline view">
          {VIEWS.map((v) => (
            <button
              key={v.key}
              role="tab"
              aria-selected={view === v.key}
              className={`tt-switch-btn${view === v.key ? ' is-active' : ''}`}
              data-testid={`time-view-${v.key}`}
              title={v.hint}
      ...
      {drawn && (
        <div className="tt-totals">
          ...
          {view === 'tape' && idleMs >= NOTE_FLOOR_MS && (
            <span className="tt-total-side" data-testid="time-timeline-idle">空闲 {formatDuration(idleMs)}</span>
          )}
          {view === 'chapters' && (
            <span className="tt-total-side" data-testid="time-timeline-chaptercount">{chapterCount} 个章节</span>
          )}
      ...
      {(day?.degraded || dayMerged?.degraded) && (
        <div className="time-degraded">Showing a partial day: the read gave up before it finished.</div>
      )}
      ...
      {drawn && view !== 'lanes' && (
        <div className="tt-scroll" ref={scrollRef} style={{ maxHeight: MAX_SCROLL_PX }}>
          {view === 'tape'
            ? (
              <TimeTape
                slices={slices}
 ...
 * minutes — a caption about 8 seconds is noise, not honesty.
 */
function NotDrawnNote({ day }: { day: DayBlocks }) {
  ...
  if ((day.overlapMs ?? 0) >= NOTE_FLOOR_MS) {
    notes.push(`${formatDuration(day.overlapMs!)} ran alongside other work (two windows?)`);
  }
