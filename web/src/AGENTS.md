...
- **Session panel on home page**: Clicking any session pill in the ToDo panel opens a resizable side panel (`SessionPanel.tsx`) between chat and ToDo with full session chat. Panel width persisted to localStorage via `useResizablePanel` hook. Header shows live status via WS events. Input hidden for terminal sessions.
