...
The user wants you to learn a reusable skill from their request and save it.
...
Do this:
1. Gather every source the user named with your own tools: read local files, fetch URLs, use the current conversation history if they referred to something you just did, and take pasted text as-is. For a codebase big enough to need its own agent, hand the work to a session with `session_start`. If scope is ambiguous, make a reasonable choice and note it; do not stall.
...
3. Confirm the skill name + category + one-line description with the user (creating a NEW skill always requires confirmation), then write ONE skill:
   ```
   walnut tools call api '{"method":"POST","path":"/api/v1/skills","body":{"dirName":"<name>","category":"<category>","content":"---\nname: …\n---\n\n# …"}}'
   ```
   `content` is the whole SKILL.md, frontmatter included. This always writes the Walnut-managed skills dir, which is the git-synced one.

Follow these authoring standards exactly (HARDLINE — same rules a maintainer enforces in review):

**Frontmatter (the `---` block at the top of `content`):**
- name: lowercase-hyphenated, ≤64 chars, no spaces.
- description: ONE sentence, **≤60 characters**, ends with a period. State the capability, not the implementation. No marketing words (powerful, comprehensive, seamless, robust). Do NOT repeat the skill name. This rule is NOT cosmetic: the skill index injects the description into every session and routing depends on it, so anything vague or over-long breaks routing. COUNT the characters before saving.
...
- category: pick a sensible grouping (finance, career, projects, walnut, ...). Reuse an existing category when one fits. It is a request field, not frontmatter: it becomes the skill's parent directory.
