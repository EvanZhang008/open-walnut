...
export const tools: ToolDefinition[] = [
  ...
  {
    name: 'task_create',
    description: `Add a task (or an empty project) to the USER's board without starting any work or session.

The board is the user's, and you are their dispatcher: a task appears on it because the user asked to record or track something, or because work is blocked on a decision or action only they can take. Follow-up work you discovered yourself (a missing test, an open investigation, a guard to add) is done now, in this conversation or in the session already working on it, never parked as a task.
  ...
  // ── Session Tools ──
  {
    ...
    async execute(params) {
      ...
      if (runnerFilter === 'all' || runnerFilter === 'cli') {
        ...
        // thread), not a user-launched session — the same rule every session
