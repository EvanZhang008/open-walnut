...
- type=task (default): Create a task. Category MUST exist (error if not). Project MUST exist if specified and different from category (error if not). Use parent_task_id for child tasks (inherits category, project, source from parent — skips existence checks).`,
    input_schema: {
      ...
      properties: {
        ...
        // Task fields
        title: { type: 'string', description: 'Task title. Required for type=task. Format: "<≤3-word prefix> — <short description>". Prefix MUST be the most unique identifier of the task (max 3 words) — the thing that instantly tells you WHICH task this is. Use em-dash (—). Good: "Sprint picker — query/select current sprint", "Task list no-jump — clicking a task does not scroll to its position". Bad: generic prefixes like "Sprint enhancements", "Bug:", "Tool Description".' },
...
For categories (type='category'): rename a category across all tasks (requires old_name and new_name).`,
    input_schema: {
      ...
      properties: {
        ...
        title: { type: 'string', description: 'New title. Format: "<≤3-word prefix> — <short description>". Prefix MUST be the most unique identifier of the task (max 3 words) — the thing that instantly tells you WHICH task this is. Use em-dash (—). Good: "Sprint picker — query/select current sprint", "Task list no-jump — clicking a task does not scroll to its position". Bad: generic prefixes like "Sprint enhancements", "Bug:", "Tool Description".' },
