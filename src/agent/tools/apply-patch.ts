/**
 * apply_patch tool — applies file patches using the *** Begin Patch / End Patch format.
 * Ported from moltbot with framework coupling removed.
 */
...
export function createApplyPatchTool(cwd?: string): ToolDefinition {
  ...
  return {
    ...
    async execute(params) {
      ...
      if (!input?.trim()) {
        return "Error: Provide a patch input.";
      }
      try {
        const result = await applyPatch(input, { cwd: resolvedCwd });
        return result.text;
      } catch (err) {
...
export async function applyPatch(
  ...
  for (const hunk of parsed.hunks) {
    if (hunk.kind === "add") {
      const target = resolvePatchPath(hunk.path, options);
    ...
    if (hunk.movePath) {
      const moveTarget = resolvePatchPath(hunk.movePath, options);
...
function resolvePatchPath(
  filePath: string,
  options: ApplyPatchOptions,
): { resolved: string; display: string } {
