    ...
    if (record?.lane) {
      void (async () => {
        try {
          ...
          // LLM auto-title: the default titler reads chat-history, which is EMPTY
          // for lanes (transcript lives in the CLI JSONL) — feed it the message
          // text directly. One user message is enough for a label (minMessages 1).
          // The current title rides along as context: mid-thread it carries the
          // first message's signal, so a bare "continue" can't become the label.
          // Self-dedups via titleAutoGenerated (the raw deriveTitle placeholder
          // touchLaneConversation just wrote does NOT set the flag, so it gets
          // upgraded exactly once); emits its own CONVERSATION_UPDATED on rename.
          const { generateConversationTitle } = await import('../../core/conversation-title.js')
          const titleContext = conversation && conversation.title !== 'New Conversation'
            ? `[Conversation so far: ${conversation.title}]\n`
            : ''
          void generateConversationTitle(ids.agentId, ids.conversationId, {
            transcript: `${titleContext}User: ${data.message as string}`,
            minMessages: 1,
          })
        } catch (err) {
