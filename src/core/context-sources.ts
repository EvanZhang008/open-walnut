...
import type { AgentDefinition, ContextSourceId, Task } from './types.js';
...
async function loadProjectMemory(task: Task, budget: number): Promise<string> {
  ...
  const { getProjectMemory } = await import('./project-memory.js');
...
async function loadGlobalMemory(budget: number, agentId?: string): Promise<string> {
  const { getMemoryFile } = await import('./memory-file.js');
...
async function loadDailyLog(budget: number, agentId?: string): Promise<string> {
  const { getDailyLogsWithinBudget } = await import('./daily-log.js');
...
async function loadWorkingMemory(budget: number): Promise<string> {
  const { getWorkingMemory, isWorkingMemoryEmpty } = await import('./working-memory.js');
  let content: string | null = null;
  try {
    const { getMainConversationId } = await import('./conversations.js');
