...
defineOp({
  ...
  description:
    'Escape hatch: perform one raw HTTP request against the local Walnut server for anything ' +
    'the named ops don\'t cover yet. `path` must start with /api/ (e.g. /api/v1/routines). ' +
    'Prefer a named op when one exists — this passthrough has no curated semantics.',
  input: {
