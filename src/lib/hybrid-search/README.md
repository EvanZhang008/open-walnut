...
- **Contentless FTS5** (`db.ts`). `doc` holds raw text (snippets, rescoring, rebuilds); `doc_fts` (`content=''`, `contentless_delete=1`) indexes the token streams with `tokenchars '-_.'` so joined identifiers survive as single FTS tokens. Eight columns: one orig stream per field (title 10, summary 3, note 1, meta 2) and one sub stream per field at ~60% of the orig weight (6 / 1.8 / 0.6 / 1.2, derived from the orig weights so the ratio cannot drift) — a subword hit in a title outranks a whole-word hit in a body, and phrases can never chain across fields. bm25() hardcodes k1/b and normalizes by WHOLE-ROW length, so a one-token title saturates the per-phrase ceiling on a single hit while a long body is penalized 6-28x: a bare-title stub is the bm25 best whatever the column weights are. No weight vector fixes that, and lowering title measurably HURT single-token component queries, so these weights are unchanged and `bodyCoverage` in `query.ts` carries the fix instead. A tokenizer or FTS-layout version bump re-tokenizes from the stored doc rows (seconds); only a doc-schema bump forces a full source re-feed.
...
- **Two query lanes + additive scoring** (`query.ts`). A strict AND lane for precision (CJK via ordered-bigram phrases), a relaxed OR lane for recall with a document-frequency threshold so corpus-wide terms cannot melt latency. Scores are a weighted sum of normalized BM25 (both lanes) 0.3 / 0.2, term coverage 0.2, **body coverage 0.2**, exact-identifier hits, recency, and (when enabled) cosine similarity, with every component exposed on the hit. The first four sum to 0.9. Body coverage is the fraction of DISCRIMINATIVE query terms (those that passed the df gate) present in the summary/note streams: presence, not count, so row-length normalization has no grip on it, and skipping high-df terms keeps it from degrading into a document-length bonus. It is off for queries under 3 terms, where a handle lookup should let a bare task win.
...
- **Embedding off the main thread** (`embedder.ts` + `embed-worker.ts`). The ONNX model runs in a `worker_thread`; `searchSemantic()` races the query embed against a deadline and falls back to keyword order with a `semantic: 'timeout'` marker. Three consecutive worker crashes disable the semantic lane for the process. Passages come from `chunk.ts`, and EVERY kind is chunked: `passagesForDoc` returns a digest at seq 0 (title + summary + the head of the body, so the recall lane — which reads only seq 0 — can still discover a long doc) plus cover passages at seq 1..n that contain every remaining character. Passages are sized by an estimated TOKEN budget (480, under the tokenizer's 512) as well as a 1400-char ceiling, because CJK runs about one token per character and a char-only budget silently truncated Chinese passages inside the embedder. A doc whose whole text fits one passage collapses to a single vector (`coverFrom: 0`). Over `MAX_CHUNKS_PER_DOC` the cap drops text and reports how much as `droppedChars`; `overflow: 'tail'` keeps the newest chunks for chronological bodies, the default `'spread'` keeps both ends for structured ones. `PASSAGE_POLICY_VERSION` is stamped into the index and a mismatch at open drops `doc_vec` (keyword index survives) so the backfill re-embeds under the new layout. `backfillVectors()` embeds docs missing vectors in small batches so an indexer loop can pace itself; its walk is bounded by a scan budget per call (`MISSING_VEC_SCAN_LIMIT`) and takes an optional `minUpdatedAt` floor, so an indexer that has already drained once asks about a timestamp range instead of sweeping the table.
...
// hits[0].components → { bm25Strict, bm25Relaxed, coverage, bodyCoverage,
//                        exactIdent, selfIdent, recency, cosine? }
