...
// open-walnut-remote.sh is embedded as a string to avoid file path issues with bundlers.
...
# open-walnut-remote.sh — Independent remote session runner for Open Walnut.
...
REMOTE_DIR="\${OPEN_WALNUT_REMOTE_DIR:-/tmp/open-walnut-streams}"
ACTION="\${1:-}"
SESSION_ID="\${2:-}"

if [ -z "$ACTION" ] || [ -z "$SESSION_ID" ]; then
  echo "Usage: open-walnut-remote.sh {start|status|stop} <session-id> [cwd] [claude-args...]" >&2
  exit 1
fi
...
const REMOTE_STREAMS_DIR = '/tmp/open-walnut-streams'
...
export class RemoteIO implements SessionIO {
  ...
  /** Remote log file path — structured debug log written by open-walnut-remote.sh. */
   ...
   *   Step 1: Deploy open-walnut-remote.sh to remote (~50 lines, <0.1s)
  ...
  setupRemote(claudeArgs: string[], cwd?: string, initialMessage?: string, append?: boolean): {
    ...
    // ── Step 1: Deploy open-walnut-remote.sh to remote ──
    ...
    const scriptPath = `${remoteDir}/open-walnut-remote.sh`
    ...
    try {
      ...
      log.session.debug('RemoteIO: deployed open-walnut-remote.sh', { host: this.host })
    } catch (e) {
      log.session.error('RemoteIO: failed to deploy open-walnut-remote.sh', {
        host: this.host,
        error: e instanceof Error ? e.message : String(e),
      })
  ...
  async checkRemoteAlive(): Promise<'running' | 'dead' | 'no-pgid' | 'error'> {
    ...
    const scriptPath = `${REMOTE_STREAMS_DIR}/open-walnut-remote.sh`
