...
- **Service links open in the Web view**: a click on an http(s) link in a session's chat column whose host is loopback (`localhost`, `127.x`, `0.0.0.0`, `::1`), the session's host, or any configured host (with a port) opens `SessionWebView` in the split instead of a browser tab. The decision is made at CLICK time in `SessionPanel` (`handleServiceLinkClick` on `.session-panel-chat-col`, classifier `web/src/utils/service-link.ts`), so rendered HTML stays a plain `<a target=_blank>` everywhere; ⌘/Ctrl/Shift/middle clicks, anchors marked `data-native-link`, and a console not served from loopback keep native behavior. The server (`POST /api/sessions/:id/service-preview`, `src/core/session-service-preview.ts`) picks the serving host, opens an SSH forward through `DaemonConnection.ensurePortForward(port, target)` for a remote one, probes the page (refused vs slow, X-Frame-Options / frame-ancestors), and answers the 127.0.0.1 URL the iframe loads. Loopback/host matching rules live in ONE zero-import module shared by both sides: `src/core/service-url.ts` (`@open-walnut/service-url`). The kebab's "Web preview" opens the view empty for a typed address (`8080`, `:8080/x`, `host:port`). Ratchets: `tests/web/routes/sessions-service-preview.test.ts`, `tests/e2e/browser/session-service-preview.spec.ts`.
...
- **Live-tail guard**: while streaming, the last MAIN-lane block (`lastMainLaneIndex`) is never matched: its partial content / early-known msgId must not hide it mid-accumulation. Exception: an Agent/Task tool_call tail IS matched (exact toolUseId; its persisted row is the anchor we want, see the subagent bullet below).
- **Subagents follow the Claude Code model: the lane never renders in the main conversation** (`web/src/stream/group-blocks.ts`, 2026-09-08). One compact Agent card sits at the spawn position (streaming `StreamingTaskGroup` while the tool_call is in the buffer, collapsed by default; the persisted `TaskGroup` the moment history absorbs it), the Background ledger (`WorkflowProgress`, pinned above the scroll area, fed by `session:background-tasks`) carries the run with per-agent elapsed/tokens/tool-uses/activity and "View transcript", and every lane block (any `parentToolUseId`) is CONSUMED: inside the card when opened, nowhere when its anchor is hidden or gone. There is no "orphan group" any more, and a groupable parent absorbs on its bare toolUseId twin like any tool_call (`promote-blocks.ts`). Two designs preceded this and both put the same agent on screen twice: an anonymous "Subagent (continued)" box at the tail (inc-1785965937858), then a labeled live duplicate kept "as the safe direction" while the ✓ history card already rendered (the user could not tell which was current). Lane blocks of a running agent stay HELD silently (the ledger reads them; `allBlocksAbsorbed` waits for `bgTaskFinished`), just not rendered. Pinned by `tests/web/chat-lab/scenarios.test.ts`, `tests/web/stream/group-blocks-nested-lane.test.ts`, `tests/e2e/browser/subagent-interleave.spec.ts`.
