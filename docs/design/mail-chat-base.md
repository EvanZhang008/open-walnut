...
MailProviderSpec {
  ...
  bodyRevision?: string                    // bump it and the base drops the bodies it cached from here
  identityRevision?: string                // bump it and the base drops every cached row and re-lists
  listUnread?(accountId, mailbox, limit)   // optional: the server's unread set now; ingest-only
...
- **A provider can retire the bodies the base cached from it.** The base stores every fetched body on disk and serves it forever, which is right for bytes that cannot change; what does change is the provider's own reading of them, and a fix to that (a helper that decoded every message as the wrong charset, a parser that dropped the plain-text half) leaves every body fetched before it wrong in the cache with no read path that will ever ask again. `MailProviderSpec.bodyRevision` (contract 1.7.0) is a string only the provider gives meaning to: a value the base has not seen makes it drop every body it cached from that provider's accounts once, keeping the envelopes, and each body is fetched again the next time that message is opened. It is recorded per provider in the plugin's `meta` table and written only after the sweep finishes, so a crash part way through repeats the sweep rather than declaring it done. Undefined means never, and only the provider may bump it: the base cannot tell a re-decoded body from the same body twice.
- **A provider can retire the identity of everything the base cached from it.** The cache is keyed by `messageId`, which is right while a provider's handles are stable and wrong the day the provider changes what a handle means (a conversation api that keyed rows by thread id and now keys them by thread in folder, so one thread with an unread item in the Inbox and a copy in Sent no longer collapses into one row that flips folder on every poll). Every row written under the old scheme is then a ghost no poll matches again. `MailProviderSpec.identityRevision` (contract 1.8.0) is the heavier sibling of `bodyRevision`: a value the base has not seen drops every cached message of that provider's accounts (rows, body files, search index) and forgets every mailbox cursor, once, so the next poll lists each mailbox from the top under the new handles. Accounts, mailboxes, drafts and the approval ledger stay; a draft's `inReplyTo` is a provider handle the provider keeps honouring across its own revision. Recorded per provider in `meta`, written after the sweep, so a crash repeats it.
- **The unread filter asks the provider, then reads the cache.** A folder's unread count comes from the provider on every tick, but a row's flag is learned only when a poll page covers it, so a message read or marked unread on another device, or one the backfill has not reached, made the badge and the filtered list disagree. `MailProviderSpec.listUnread(accountId, mailbox, limit)` (contract 1.9.0, optional) returns the unread envelopes of one mailbox as the server sees them now; the first unread page of `GET /messages` ingests that answer (8 s deadline, failure leaves the cache as it is) before it reads the cache. Ingest-only by contract: nothing is removed and an empty array adds nothing, which is what lets a provider answer for the one folder its api can filter and return [] for the rest.
...
plugin.sqlite (mail)
  ...
  messages(account_id, message_id, rfc_message_id, mailbox_id, thread_id,
           ...
           seen, attachments_json, body_ref, body_bytes, payload)
                                  // index on (account_id, mailbox_id, seen, sent_at DESC)
...
- **`seen` is a derived column, and the only flag that gets one**: `flags_json` stays the truth (a provider's other flags live there and nothing queries them), but "only unread" is asked often enough to need an index, and a JSON array cannot be one. The two ways to read it from SQL are each wrong for a page query: `LIKE '%\Seen%'` is a substring match on the text, so a keyword like `$Seen` reads as read, and an exact `json_each` subquery is unindexable, so it re-parses every row in the mailbox until the page fills (measured at 50k rows with unread sparse: 11.2ms against 0.017ms, and growing with the mailbox rather than with the page, which makes paging down a big folder quadratic). The three statements that write flags move the column in the same breath, so the two can never disagree.
...
A list read is one keyset-paged page of a mailbox, newest first, and the cursor it hands back is opaque: `GET /messages?account=&mailbox=&limit=&before=`, with `before` carrying the whole sort key rather than a timestamp, because the key is not unique and a page that ended inside a group of equal `Date` headers used to skip the rest of that group forever. `unread=1` narrows the same query in SQL and pages the same way, so "only unread" means the mailbox rather than the fifty rows a client happens to hold: filtering in the browser showed "4 unread" under a folder row reading "Inbox 99+" and offered those four. Exactly `'1'` turns it on, so a typo cannot silently hide most of somebody's mail. The mailbox's own unread COUNT stays the provider's number, refreshed on a mailbox re-list and nudged locally when Walnut itself moves a flag, and it is what the console's header shows.

Every count a person sees for one folder comes from that one mailbox row: the folder badge on the left, the folder size in the list header, and the header's unread chip. They are printed exactly and grouped by locale (12,345), never capped, because the cap is what made the numbers unreadable: a folder row saying "Inbox 99+" beside a header saying "INBOX 50" and "5 unread" gave three figures for one folder and no way to tell which was lying. Only two things are page-derived, and both say so in a `title`: the first paint before any mailbox list has landed, and a provider that declares fewer messages than the console is already holding (a claim that cannot be true, so the number that can be counted on screen wins). Moving a read flag moves the row, the badge, the chip and the account total in one patch, so they cannot drift apart between renders.
...
The console renders one generic account form for every provider: the provider declares `setup.fields` (text, password, select, with help strings), the console posts the values to the base, and the base passes them straight to `provider.setup.submit(values)` **without persisting them**. The provider stores its own config and secrets and returns the account record. Reads never return secret values, only `configured: true`. A provider may also declare `setup.presets`, the known services behind it (Gmail, iCloud, Outlook.com, Fastmail, Yahoo for IMAP): each one may carry the address domains it is for plus the server values, so typing an address fills the servers and shows that vendor's own credential or setup page, while anything the person typed by hand is never overwritten and "Other" only stops the guessing. A preset fills the host and the encryption choice and deliberately no port, because `submit` derives the port from that choice and the two can then never disagree.
