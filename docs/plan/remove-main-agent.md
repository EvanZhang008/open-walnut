...
no task, promote-to-task by hand              Ask Walnut task; tab strip to switch;
                                              New = Ask Walnut composer in the slot
...
1. Console agents (mentor, note-agent, custom) move to the same task-backed model in P3: the quick-start `walnutAgent` flag generalizes to an `agentId`, each agent gets its own project (for example `Ask Mentor`), and `consoleAgentProfile` is kept as is. Keeping the lane alive only for them was rejected because the removal would not be complete.
...
- New `AskWalnutSlot` replaces both chat implementations in `MainPage.tsx` (the in-process `ChatPanel` branch and the lane `SessionChatHistory` branch). It derives its list from the task store (`walnut_agent === true`, sorted by `updated_at` descending), resolves each task's newest session from the session list, and renders a `SessionPanel` for the selected task in embedded mode (no lock, popout, or close controls; the rest of the panel unchanged).
- Header strip: `Ask Walnut` title, tab strip of recent tasks (overflow into a placed menu), `New`, and the surviving header actions (`+ Task`, `+ Session`, `Fix Walnut`, context inspector, close chat). `Promote to task`, engine badge, engine switch, and the chat stats pill are removed (obsolete or already superseded on the session path).
- `New` renders `DraftSessionPanel` inside the slot with a synthetic walnut draft (the same Ask Walnut tab users already know: copy, one-tap seeds, composer, model pill locked to claude). Start calls `quickStartSession({ walnutAgent: true })` directly, shows a pending state in the slot, and selects the new task as soon as the response carries `taskId` and `sessionId`. No session column is opened for slot launches.
...
Verification: Playwright against the real UI (chromium and webkit, the Mac app is a WKWebView): empty state shows the composer, a launch creates a tab and streams the mock reply inside a `SessionPanel`, `New` then a second launch yields two tabs, switching tabs switches history, selection survives a reload, `Fix Walnut` still opens a pre-armed draft, mobile width (390px) keeps the slot usable.
