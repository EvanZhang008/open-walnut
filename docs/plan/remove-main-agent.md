...
- New `AskWalnutSlot` replaces both chat implementations in `MainPage.tsx` (the in-process `ChatPanel` branch and the lane `SessionChatHistory` branch). It derives its list from the task store (`walnut_agent === true`, newest created first), resolves each task's newest session from the session list, and renders the regular `SessionPanel` for the selected task in embedded mode (no lock, popout, or close controls; the rest of the panel, header and chips included, unchanged).
- The one addition to the panel is a ≡ button at the top-left of its header (user direction, 2026-09-07: "just reuse the regular panel, add a Claude-style switcher top-left"). It slides a drawer in from the left over the panel: quick actions on top (`+ Task`, `+ Session`, `Find sessions`, `Fix Walnut`, `Context`, `Hide Ask Walnut`), the recent asks below with the current one marked and a live status dot, and `New chat` pinned to the bottom. Picking anything closes the drawer. `Promote to task`, engine badge, engine switch, and the chat stats pill are removed (obsolete or already superseded on the session path).
...
Verification: Playwright against the real UI (chromium and webkit, the Mac app is a WKWebView): empty state shows the composer and an empty drawer, a launch streams the mock reply inside a `SessionPanel` and lists one current row in the drawer, `New chat` then a second launch yields two rows (newest on top), picking a row switches history, selection survives a reload, `Fix Walnut` still opens a pre-armed draft, mobile width (390px) keeps the drawer and composer inside the viewport.
