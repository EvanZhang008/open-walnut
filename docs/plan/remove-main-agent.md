...
Verification: Playwright against the real UI (chromium and webkit, the Mac app is a WKWebView): empty state shows the composer and an empty drawer, a launch streams the mock reply inside a `SessionPanel` (with ×, popout and fullscreen present and lock absent) and lists one current row in the drawer, `New chat` then a second launch yields two rows (newest on top), picking a row switches history, selection survives a reload, the × hides the slot and the dock brings it back, expand-to-fullscreen covers the page rather than the slot, the drawer's search filters by title, `Fix Walnut` still opens a pre-armed draft, mobile width (390px) keeps the drawer and composer inside the viewport.
