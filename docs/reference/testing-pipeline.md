...
|---|---|---|---|---|
| **L1 quick** | `npm run test:quick` | 311 pure-logic files | **51 s** (4 workers) / 93 s (2 workers) | every code change |
...
`vitest.quick.config.ts` runs everything EXCEPT the 26 files measured over 2 s (`tests/setup/slow-tests.ts`) and the two end-to-end-heavy directories. Those 26 are slow because they start something real — a `claude` CLI, a local daemon, a git subprocess, an HTTP server — so they are exactly the wrong thing to run on every save.
...
| `web/src/**` | frontend-rooted configs (plus Playwright, run separately) |
| `vitest.*.config.ts`, `tests/setup/**` | quick + slow |
| `package.json` / lockfile | quick + slow |
...
`.github/workflows/ci.yml`: a `build` gate (type-check + build), then four test tiers as a **parallel matrix** — each leg gets its own runner, so wall-clock is the slowest single tier rather than the sum. Locally the tiers are forced sequential because they share one machine; on CI parallel is free.
...
Measured 2026-07-25 against a clean clone of `main` (`e399786`): the quick tier has **119 pre-existing failures on the committed tree** — stale test imports of exports deleted in 2026-05, tests that need a real `claude` CLI or daemon, assorted contract drift, and a few load-dependent flakes. A tier with a non-zero baseline cannot be a pass/fail gate: it would paint `main` permanently red, and an always-red check teaches everyone to ignore it.
...
`tests/setup/known-failures.json` is committed, so a PR that adds entries is visibly making things worse. The slow and e2e tiers stay report-only until they get the same treatment. **Flip a tier to `blocking: true` in the workflow once its baseline reaches zero.**
...
|---|---|---|---|
| quick | 311 | 51 s @4w | the every-change layer |
