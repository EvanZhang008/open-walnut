...
**Idle timeout**: Local FIFO sessions auto-kill after 30 minutes of idle (configurable via `config.session.idle_timeout_minutes`, 0 = disabled). Graceful two-phase shutdown (SIGINT → SIGTERM) preserves session state for `--resume`. Timer is cancelled on new messages, restored on server restart. SSH sessions and `await_human_action` sessions are excluded.
