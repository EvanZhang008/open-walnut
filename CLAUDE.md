...
| **User Interaction** | `ask_question` (1-4 questions per call, blocks mid-turn until user answers in chat UI) |
| **Inline Subagent** | `create_subagent` (spawn lightweight `claude -p` subprocess, streaming output as collapsible agent box in chat) |
...
## Inline Subagent System (`src/providers/inline-subagent.ts`)

`create_subagent` spawns a lightweight `claude -p --output-format stream-json` subprocess for quick AI-assisted tasks. Unlike `start_session`, no task is required and no persistent session is created — results return directly to the main agent. Streaming JSONL output is parsed by `claude-stream-parser.ts` into `StreamingBlock` objects and emitted as `agent:subagent-stream` events to the frontend, where `SubagentBlock` renders them as collapsible agent boxes in the main chat. Supports foreground (sync, blocks tool) and background (async, returns immediately) modes. Default model: opus. Concurrent limit: 3 (semaphore). Timeout: 120s default, 600s max (two-phase kill: SIGINT → 3s → SIGTERM).
...
**Synthetic user events**: Claude Code's `--output-format stream-json` stdout does NOT echo user text messages — only `tool_result` protocol messages and assistant responses appear. To ensure the local JSONL has complete conversation history (user + assistant interleaved), `writeSyntheticUserEvent()` on `ClaudeCodeSession` appends a `{"type":"user","subtype":"walnut-injected","walnutMessageId":"..."}` event to the local output file when a message is delivered via FIFO or `--resume`. The `walnutMessageId` (= queue message ID) enables deterministic dedup of optimistic frontend copies: `SessionChatHistory` Layer 1 matches `persistedWalnutIds` against `optimistic.queueId`, removing duplicates regardless of optimistic status. Layer 2 (text-based) is the legacy fallback for sessions without synthetic events. The parser (`parseSessionMessages`) also deduplicates synthetic vs canonical user messages by text to prevent doubles when both sources contain the same message.
...
**Three-tier session state model**: `ProcessStatus = 'running' | 'idle' | 'stopped'`. Running = actively processing a turn. Idle = turn complete, process alive, waiting for input. Stopped = process dead (resumable via `--resume`). Three layers manage resources: (1) **Processing Limit** (per-host, local=7, remote=20) — blocks new sessions when too many running; (2) **Idle Limit** (per-host, local=30, remote=40, configurable via `config.session.max_idle`) — evicts oldest idle session when limit exceeded; (3) **Idle Timeout** (health monitor, file mtime, default 30min via `config.session.idle_timeout_minutes`, 0 = disabled) — checks JSONL output file mtime to detect idle sessions (survives server restarts, no state machine dependency). Kills via SIGINT → SIGTERM. Works for both local and SSH sessions. `await_human_action` sessions excluded.

**Remote orphan cleanup (3-layer defense)**: Remote sessions run via `walnut-remote.sh` which manages its own lifecycle (signal traps, PGID file, writer PID file). Layer 1: PID file (`/tmp/walnut-streams/{id}.pgid`) + writer file (`.writer`) written by the script, cleaned by HUP/TERM/INT traps + normal exit. Layer 2: `RemoteIO.gracefulStopRemote()` sends SIGINT→SIGTERM to remote claude via SSH; wired into `kill()`/`gracefulStop()`/`interrupt()`. The script's `stop` command also kills the writer process. Layer 3: health monitor sweeps all configured hosts for orphaned PID files every 30min + on startup (`RemoteIO.sweepRemoteOrphans()`).
...
**Team session support (Claude Code Teams)**: When a session uses `TeamCreate` + `Agent` tools with `team_name`, the UI renders a `TeamCard` inline in the conversation. Team detection: `handleStreamLine()` in `ClaudeCodeSession` tracks `_teamName`/`_teamAgents`; `SessionChatHistory.tsx` scans history messages for team tools via `useMemo`. The `TeamCard` shows a tab strip (one per non-lead member with ✓/● status), and clicking a tab loads the agent's conversation by polling its JSONL file. Backend: `team-reader.ts` reads `~/.claude/teams/{name}/config.json` and discovers teammate JSONL paths (tmux: scan project dir, in-process: scan subagents dir). `subagent-poller.ts` polls one agent at a time (2s local, 5s remote), with multi-file tracking and periodic new-file discovery. Chat: `writeToInbox()` writes to `~/.claude/teams/{name}/inboxes/{agent}.json`. RPCs: `session:team-info`, `session:team-agent-subscribe/unsubscribe`, `session:team-send`. Known limitation: remote sessions where team config is only on the SSH host show no TeamCard (graceful degradation). **TeamDelete fallback**: When `TeamDelete` removes the config from disk (normal at session end), `extractTeamsFromLeadJsonl()` parses Agent tool calls from the lead session JSONL to reconstruct member info, and `findSubagentJsonlByPrompt()` matches subagent files by full prompt content. `findAllSubagentJsonlsForAgent()` discovers all JSONL files per agent (main + inbox responses) for multi-file merge. Sent messages appear as optimistic blue bubbles via `addSentMessage()` in `useTeamStream`. **In-process inbox delivery**: Messages are delivered between rounds only (not mid-tool-call). Each delivery creates a NEW JSONL file with a new agentId, linked to the previous file via `parentUuid` chain. An agent receiving N inbox messages has N+1 JSONL files. The poller (`ActiveTabPoller`) re-discovers new files every ~10s via `findAllSubagentJsonlsForAgent()`. UI shows "Delivered after current turn ends" hint when agent is busy. **Tmux mode** delivers messages in real-time to the running process (separate OS processes per teammate).
...
| **Agent** | `agent:text-delta`, `agent:tool-activity`, `agent:tool-call`, `agent:tool-result`, `agent:thinking`, `agent:response`, `agent:error`, `agent:subagent-stream` |
