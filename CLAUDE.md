...
**Remote orphan cleanup (3-layer defense)**: Remote SSH sessions (claude/tail/sleep) can become orphans when the local SSH tunnel dies (no PTY → no SIGHUP). Layer 1: PID file (`/tmp/walnut-streams/{id}.pgid`) written on remote startup, cleaned by hardened EXIT trap (HUP/TERM/INT). Layer 2: `RemoteIO.gracefulStopRemote()` sends SIGINT→SIGTERM to remote claude via SSH; wired into `kill()`/`gracefulStop()`/`interrupt()`. Layer 3: health monitor sweeps all configured hosts for orphaned PID files every 30min + on startup (`RemoteIO.sweepRemoteOrphans()`).
